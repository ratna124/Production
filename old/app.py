...
def invalidate_csv_cache(*paths):
    with _csv_cache_lock:
        for p in paths:
...
_code_index    = {}   
_code_index_ts = {}  
CODE_INDEX_TTL = 60   # detik

def get_code_index(catalog_key):
    now = time.time()
    if (catalog_key in _code_index and
            now - _code_index_ts.get(catalog_key, 0) < CODE_INDEX_TTL):
        return _code_index[catalog_key]
    path = CATALOG_MAP.get(catalog_key)
    ...
    index = {row["code"].upper(): row.to_dict() for _, row in df.iterrows()}
    _code_index[catalog_key]    = index
    _code_index_ts[catalog_key] = now
    return index

def invalidate_code_index(*catalog_keys):
    for k in catalog_keys:
        _code_index.pop(k, None)
        _code_index_ts.pop(k, None)
...
def cleanup_cache():
    ...
    with _csv_cache_lock:
        expired_csv = [k for k, (_, t) in _csv_cache.items() if now - t > 300]
        for k in expired_csv:
            del _csv_cache[k]
...
def generate_code(data):
    ...
    if divisi == "AVAL_HD":
        jenis_map = {"Prong": "AHP", "Daun": "AHD", "Sapuan": "AHS",}
...
# ─── SPK LOOKUP ─────────────────────────────────────────────
def load_spk_data():
    df = read_csv_cached(SPK_CSV, encoding="utf-8-sig", on_bad_lines='skip', engine='python')
    df.columns = df.columns.str.strip()
...
def api_hasil_produksi():
    try:
        ...
        df_spk = read_csv_cached(SPK_CSV, encoding="utf-8-sig", on_bad_lines='skip', engine='python')
        ...
        def load_catalog_by_code(path, spk_col, berat_col):
            ...
            if not os.path.exists(path):
                return result
            df = read_csv_cached(path, encoding="utf-8-sig", on_bad_lines='skip', engine='python')
            df.columns = df.columns.str.strip()
...
def api_hasil_produksi_hd():
    try:
        ...
        df = read_csv_cached(CSV_HD, encoding="utf-8-sig", on_bad_lines='skip', engine='python')
        df.columns = df.columns.str.strip()
        ...
        if scan_salah_path.exists():
            ds = read_csv_cached(scan_salah_path, encoding="utf-8-sig", on_bad_lines='skip', engine='python')
    ...
    # FILTER TANGGAL
        ...
        if os.path.exists(CSV_AVAL_HD):
            da = read_csv_cached(CSV_AVAL_HD, encoding="utf-8-sig", on_bad_lines='skip', engine='python')
            ...
            sapuan_per_tgl_shift = (sub_sap.groupby([ca_tgl, ca_shift])[ca_berat].sum().to_dict())
...
def api_hasil_produksi_mixing():
    try:
        if not os.path.exists(CSV_MIXING):
            return jsonify({"rows": [], "newest_created": ""})
        df = read_csv_cached(CSV_MIXING, encoding="utf-8-sig", on_bad_lines="skip", engine="python")
        df.columns = df.columns.str.strip()
        ...
        def flt(col, param):
            ...
            if param == "tanggal":
                try:
                    ...
                    return pd.Series(False,index=df.index)
            return (df[col].astype(str).str.strip().str.contains(val, case=False, na=False))
        ...
        if os.path.exists(CSV_AVAL_MIXING):
            da = read_csv_cached(CSV_AVAL_MIXING, encoding="utf-8-sig", on_bad_lines="skip", engine="python")
            da.columns = da.columns.str.strip()
...
def api_hasil_produksi_potong():
    try:
        ...
        df = read_csv_cached(CSV_POTONG, encoding="utf-8-sig", on_bad_lines='skip', engine='python')
        df.columns = df.columns.str.strip()
        ...
        def flt(col, param):
            ...
            if param == "tanggal":
                try:
                    target = pd.to_datetime(val).normalize()
                    csv_dates = pd.to_datetime(df[col].astype(str).str.strip(),
                        dayfirst=True,
                        errors="coerce"
                    ).dt.normalize()
        ...
        if os.path.exists(CSV_AVAL_POTONG):
            da = read_csv_cached(CSV_AVAL_POTONG, encoding="utf-8-sig", on_bad_lines='skip', engine='python')
            da.columns = da.columns.str.strip()
            ca_tgl   = da.columns[0]
            ca_shift = da.columns[1]
            ca_spk   = da.columns[3]
...
def api_hasil_produksi_packing():
    try:
        ...
        df = read_csv_cached(CSV_PACKING, encoding="utf-8-sig", on_bad_lines='skip', engine='python')
        df.columns = df.columns.str.strip()
        ...
        if os.path.exists(CSV_SISA_PACK):
            ds = read_csv_cached(CSV_SISA_PACK, encoding="utf-8-sig", on_bad_lines='skip', engine='python')
            ds.columns = ds.columns.str.strip()
            cs_tgl   = ds.columns[0]
            ...
            sisa_berat = (ds_valid.groupby([cs_tgl, cs_spk, cs_shift, cs_mesin])[cs_berat].sum().to_dict())
            sisa_pack = (ds_valid.groupby([cs_tgl, cs_spk, cs_shift, cs_mesin])[cs_sisa].sum().to_dict())
        ...
        if os.path.exists(CSV_AVAL_PACKING):
            da = read_csv_cached(CSV_AVAL_PACKING, encoding="utf-8-sig", on_bad_lines='skip', engine='python')
            da.columns = da.columns.str.strip()
            ca_tgl   = da.columns[0]
            ca_shift = da.columns[1]
            ca_spk   = da.columns[3]
    ...
    try:
        ...
        def load_codes(path):
            if not os.path.exists(path):
                return set()
            try:
                df = read_csv_cached(path, dtype=str, encoding="utf-8-sig", on_bad_lines='skip', engine='python')
        ...
        def calc_stok(cat_path, bad_codes, used_codes, berat_col_idx):
            if not os.path.exists(cat_path):
                return {}
            try:
                df = read_csv_cached(cat_path, dtype=str, encoding="utf-8-sig", on_bad_lines='skip', engine='python')
                df.columns = df.columns.str.strip().str.lower()
...
def api_stok_hd():
    try:
        ...
        df = read_csv_cached(CSV_HD, dtype=str, encoding="utf-8-sig", on_bad_lines='skip', engine='python' )
        df.columns = df.columns.str.strip()
        ...
        df[c_spk]   = df[c_spk].astype(str).str.strip()
        df[c_code]  = df[c_code].astype(str).str.strip()
        df[c_berat_bersih] = pd.to_numeric(df[c_berat_bersih],errors="coerce").fillna(0)
        ...
        if scan_salah_path.exists():
            ds = read_csv_cached(scan_salah_path,dtype=str,encoding="utf-8-sig",on_bad_lines='skip',engine='python')
            ds.columns = ds.columns.str.strip()
        ...
        if scan_pemakaian_path.exists():
            dp = read_csv_cached(scan_pemakaian_path, dtype=str, encoding="utf-8-sig", on_bad_lines='skip', engine='python')
        ...
        if spk_filter:
            rows = [r for r in rows if spk_filter in str(r["spk"]).lower()]
        if customer_filter:
            rows = [r for r in rows if customer_filter in str(r["customer"]).lower()]
        if produk_filter:
            rows = [r for r in rows if produk_filter in str(r["produk"]).lower()]
        if uk_filter:
            rows = [r for r in rows if uk_filter in str(r["uk"]).lower()]
...
def api_stok_potong():
    try:
        ...
        df = read_csv_cached(CSV_POTONG, dtype=str, encoding="utf-8-sig", on_bad_lines='skip', engine='python')
        df.columns = df.columns.str.strip()
...
def api_stok_packing():
    try:
        ...
        df = read_csv_cached(CSV_PACKING, dtype=str, encoding="utf-8-sig", on_bad_lines='skip', engine='python')
        df.columns = df.columns.str.strip()
...
def api_stok_mixing():
    try:
        ...
        df = read_csv_cached(CSV_MIXING, dtype=str, encoding="utf-8-sig", on_bad_lines='skip', engine='python')
        df.columns = df.columns.str.strip()
...
def api_stok_sisapack():
    try:
        ...
        df = read_csv_cached(CSV_SISA_PACK, dtype=str, encoding="utf-8-sig", on_bad_lines='skip', engine='python')
        df.columns = df.columns.str.strip()
...
def api_mutasi_mixing():
    try:
        ...
        with open(
            ...
            writer = csv.DictWriter(f,fieldnames=CSV_MUTASI_MIXING_COLUMNS)
        ...
        if file_exists:
            try:
                ...
                headers = ["tanggal", "shift", "divisi", "spk", "customer", "produk", "uk", "operator_mix", "checker", "mesin", "berat_kg", "berat_bersih", "karung", "created_at", "code"]
        else:
            headers = ["tanggal", "shift", "divisi", "spk", "customer", "produk", "uk", "operator_mix", "checker", "mesin", "berat_kg", "berat_bersih", "karung", "created_at", "code"]
        ...
        invalidate_csv_cache(CSV_MIXING)
        invalidate_code_index("MIXING")
...
def api_mutasi_hd():
    try:
        ...
        with open(path, "a", newline="", encoding="utf-8-sig") as f:
            ...
            if file_exists:
                try:
                    ...
                    headers = ["tanggal", "shift", "divisi", "spk", "customer", "produk", "uk", "operator_hd", "checker", "mesin", "berat_kg", "berat_bersih", "bobin", "created_at", "code"]
            else:
                headers = ["tanggal", "shift", "divisi", "spk", "customer", "produk", "uk", "operator_hd", "checker", "mesin", "berat_kg", "berat_bersih", "bobin", "created_at", "code"]
        ...
        invalidate_csv_cache(CSV_HD)
        invalidate_code_index("HD")
    ...
    try:
        ...
        try:
            if os.path.exists(SPK_CSV):
                ...
                spk_col = next((c for c in df_spk.columns
                        if "spk" in c.lower()),
                    df_spk.columns[1]
                )
                df_spk[spk_col] = (df_spk[spk_col].astype(str).str.strip())
        ...
        with open(path, "a", newline="", encoding="utf-8-sig") as f:
            ...
            if file_exists:
                try:
                    ...
                    print("Gagal baca header:",e)
                    headers = ["tanggal", "shift", "divisi", "spk", "customer", "produk", "uk", "operator_cu", "checker", "mesin", "berat_kg", "berat_bersih", "keranjang", "created_at", "code"]
            else:
                headers = ["tanggal", "shift", "divisi", "spk", "customer", "produk", "uk", "operator_cu", "checker", "mesin", "berat_kg", "berat_bersih", "keranjang", "created_at", "code"]
            ...
            with open(CSV_POTONG, "a", newline="", encoding="utf-8-sig"
            ) as mf:
                writer = csv.DictWriter(
                    mf,
                    fieldnames=headers,
        ...
        invalidate_csv_cache(CSV_POTONG)
        invalidate_code_index("POTONG")
...
def api_mutasi_packing():
    try:
        ...
        with open(path, "a", newline="", encoding="utf-8-sig") as f:
            ...
            if file_exists:
                try:
                    ...
                    headers = ["tanggal", "shift", "divisi", "spk", "customer", "produk", "uk", "operator_pa", "checker", "mesin","berat_bersih", "created_at", "code"]
            else:
                headers = ["tanggal", "shift", "divisi", "spk", "customer", "produk", "uk", "operator_pa", "checker", "mesin", "berat_bersih", "created_at", "code"]
        ...
        invalidate_csv_cache(CSV_PACKING)
        invalidate_code_index("PACKING")
...
def api_mutasi_sisapack():
    try:
        ...
        with open(path, "a", newline="", encoding="utf-8-sig") as f:
            ...
            if file_exists:
                try:
                    ...
                    headers = ["tanggal", "shift", "divisi", "spk", "customer", "produk", "uk", "operator_sp", "checker", "mesin", "berat_bersih", "created_at", "code"]
            else:
                headers = ["tanggal", "shift", "divisi", "spk", "customer", "produk", "uk", "operator_sp", "checker", "mesin", "berat_bersih", "created_at", "code"]
        ...
        invalidate_csv_cache(CSV_SISA_PACK)
        invalidate_code_index("SISA_PACK")
...
def lookup_code():
    try:
        ...
        if not os.path.exists(catalog_path):
            return jsonify(found=False, prefix=prefix, divisi_label=divisi_label,
                           csv_file=csv_file, error="File katalog tidak ditemukan")

        index = get_code_index(catalog_key)
        r = index.get(code)
        if not r:
            return jsonify(found=False, prefix=prefix,
                           divisi_label=divisi_label, csv_file=csv_file)
...
def recent(divisi):
    try:
        ...
        df = read_csv_cached(path, encoding="utf-8")
        df = df.fillna("").iloc[::-1]
